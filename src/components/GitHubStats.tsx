...
export async function GitHubStats({ repo }: GitHubStatsProps) {
  ...
  if (!data) {
    return (
      <a
        ...
        className="inline-flex items-center gap-2 px-4 py-2 border border-border rounded-lg hover:bg-accent transition-colors not-prose"
      >
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noopener noreferrer"
      className="inline-flex items-center gap-4 px-4 py-2 border border-border rounded-lg hover:bg-accent transition-colors not-prose"
    >
      <span className="font-medium font-mono text-sm">{repo}</span>
