...
Using the `CodeBlock` component directly with `showLineNumbers={false}`:
...
Using `maxLines` to limit the visible lines with an expand button:

<CodeBlock language="typescript" filename="ListenButton.tsx" maxLines={8}>
